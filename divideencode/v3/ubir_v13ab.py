"""UBIR V1.3AB: combined compact literals + punctuation.

This is an isolated ablation candidate. V1.1 remains frozen and V1.3A/B
remain unchanged. It combines only the two independently measured wins.
"""
from __future__ import annotations
from . import ubir_v11 as base
from . import universal_binary_ir as v1
TRUE,FALSE,NULL=5,6,7
PBASE=8
PUNCTS=b'{}[],: '
PMAP={c:i for i,c in enumerate(PUNCTS)}
...
def encode_json(data:bytes)->bytes:
    ...
    for t in tokens:
        if t[:1] in b'{}[],:': out.append(PBASE+PMAP[t[0]])
        elif t[:1]==b'"':
...
def decode_json(payload:bytes)->bytes:
    ...
    for _ in range(count):
        ...
        else:raise ValueError('unknown UBIR V1.3AB tag')
    if p!=len(payload):raise ValueError('trailing UBIR V1.3AB bytes')
    return bytes(out)

def encode(data:bytes,kind:str)->bytes:
    if kind!='json':raise ValueError('UBIR V1.3AB currently supports JSON only')
    payload=encode_json(data);return v1.MAGIC+bytes([v1.VERSION,v1.K_JSON])+v1._u(len(data))+payload

def decode(blob:bytes)->bytes:
    if len(blob)<6 or blob[:4]!=v1.MAGIC or blob[4]!=v1.VERSION or blob[5]!=v1.K_JSON:raise ValueError('invalid UBIR V1.3AB blob')
    raw,p=v1._r(blob,6);out=decode_json(blob[p:])
    if len(out)!=raw:raise ValueError('UBIR V1.3AB length mismatch')
    return out
