"""UBIR V1.3CF: exact decimal/scientific numeric transform.
The numeric value is typed, but the original lexical spelling is retained in
an optional sidecar so decode is byte-exact. The transform is selected only
when its complete encoded size beats the original token representation.
"""
...
def splitnum(t):
 ...
 exp=0 if es is None else (-1 if es==b'-' else 1)*int(ed)
 scale=len(frac)-exp; digits=(d+frac).lstrip(b'0') or b'0'
...
def enc_num(t):
 ...
 s,d,e=x
 # Keep original lexical spelling as a sidecar: this guarantees exact bytes.
 typed=bytes([NUMTAG,s])+v1._s(d)+v1._u(v1._zz(e))
 side=v1._s(t)
 q=typed+side
 return q if len(q)<1+len(side) else None
...
def decode(blob:bytes)->bytes:
 if len(blob)<6 or blob[:4]!=v1.MAGIC:raise ValueError('invalid UBIR V1.3CF blob')
 ...
 for _ in range(count):
  ...
  if base.PBASE<=tag<base.PBASE+len(base.PUNCTS):out.append(base.PUNCTS[tag-base.PBASE])
   ...
   if p>=len(blob):raise ValueError('truncated numeric transform')
   p+=1;_,p=v1._g(blob,p);_,p=v1._r(blob,p);lex,p=v1._g(blob,p);out+=lex
  else:raise ValueError('unknown V1.3CF tag')
 if p!=len(blob):raise ValueError('trailing UBIR V1.3CF bytes')
