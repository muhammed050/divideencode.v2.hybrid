...
Benchmark candidate only. It separates structural, key, string, and integer
information so DE2 can exploit repeated symbols and numeric locality.
"""
...
K_OBJ=1; K_ARR=2; K_STR=3; K_INT=4; K_FLOAT=5; K_TRUE=6; K_FALSE=7; K_NULL=8
...
def lex(d):
    ...
    while p<len(d):
        ...
        if c in b"{}[]:,":t=c;p+=1
            ...
            m=NUM.match(d,p)
            if not m:raise ValueError(f"invalid JSON token at {p}")
            t=m.group();p=m.end()
        out.append((ws,t))
    return out,b""
...
class P:
    ...
    def val(self):
        ...
        if t==b'{':
            ...
            while True:
                kws,key=self.take();cws,col=self.take()
                if col!=b':':raise ValueError("expected colon")
                v=self.val();comma=None
                if self.peek()==b',':comma=self.take()[0]
                fs.append((kws,key,cws,v,comma))
                if comma is None:
                    c=self.take();return('o',ws,fs,c[0])
        if t==b'[':
            vs=[]
            ...
            while True:
                ...
                vs.append((v,comma))
                if comma is None:
                    c=self.take();return('a',ws,vs,c[0])
...
def collect(n,keys,strings,ints):
    if n[0]=='o':
        for kw,k,cw,v,c in n[2]:keys.append(k);collect(v,keys,strings,ints)
    elif n[0]=='a':
...
def enc_node(n,kid,sid,ints):
    k=n[0]
    o=bytearray(bytes([{'o':K_OBJ,'a':K_ARR,'s':K_STR,'i':K_INT,'f8':K_FLOAT,'t':K_TRUE,'f':K_FALSE,'n':K_NULL}[k]])+b(n[1]))
    if k=='o':
        o+=u(len(n[2]))
        for kw,key,cw,v,c in n[2]:
            o+=b(kw)+b(cw)
            if key in kid:o+=bytes([1])+u(kid[key])
            else:o+=bytes([0])+b(key)
            o+=enc_node(v,kid,sid,ints)+b(c or b'')
        o+=b(n[3])
    elif k=='a':
        o+=u(len(n[2]))
        ...
        if n[2] in sid:o+=bytes([1])+u(sid[n[2]])
        else:o+=bytes([0])+b(n[2])
    elif k=='i':o+=u(len(ints));ints.append(n[2])
    elif k=='f8':o+=b(n[2])
    return bytes(o)

def encode(d):
    t,tr=lex(d);root=P(t).val();keys=[];strings=[];ints=[];collect(root,keys,strings,ints)
    kc=Counter(keys);sc=Counter(strings)
    kd=sorted((x for x,n in kc.items() if n>=2),key=lambda x:(-len(x)*kc[x],x))
    sd=sorted((x for x,n in sc.items() if n>=2),key=lambda x:(-len(x)*sc[x],x))
    kid={x:i for i,x in enumerate(kd)};sid={x:i for i,x in enumerate(sd)}
    ints2=[];payload=enc_node(root,kid,sid,ints2)
    # Keep the numeric stream compact and independently statistical.
    ns=bytearray();prev=0
    for x in ints2:
