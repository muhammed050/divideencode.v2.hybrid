...
def inverse(blob):
    ...
    for _ in range(count):
        if p >= len(blob):
            raise ValueError("truncated structural stream")
        kind = blob[p]
        p += 1
    ...
    for s in reversed(steps):
        data = _decode_step(Step(s.kind, s.meta, data))
    return data
...
def analyze(data, max_depth=3):
    ...
    def add(steps, names):
        ...
        if blob not in seen:
            seen.add(blob)
            candidates.append(Candidate(tuple(names), blob, len(blob)))

    for name, fn in (
        ("record-transpose", _record_transpose),
        ("column-delta", lambda x: _column_transform(x, False)),
        ("column-xor", lambda x: _column_transform(x, True)),
    ):
        x = fn(data)
        if x is None:
            continue
