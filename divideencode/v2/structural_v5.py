...
def _encode(steps):
    ...
    for s in steps:
        body.append(s.kind); body.extend(_varint(len(s.meta))); body.extend(s.meta); body.extend(_varint(len(s.payload))); body.extend(s.payload)
    return MAGIC + bytes(body)

def _raw(data): return MAGIC + bytes((VERSION, 0)) + _varint(len(data)) + data

def _record_transpose(data):
    best = None
    for width in range(2, 65):
        if len(data) < width * 32 or len(data) % width: continue
        count = len(data) // width; out = bytearray(len(data)); q = 0
        for col in range(width):
            for row in range(count): out[q] = data[row * width + col]; q += 1
        step = Step(STEP_RECORD_TRANSPOSE, bytes((width,)) + _varint(count), bytes(out)); size = len(_encode([step]))
...
def _column_transform(data, xor=False):
    ...
    for width in range(2, 65):
        if len(data) < width * 32 or len(data) % width: continue
        count = len(data) // width; out = bytearray(len(data))
        for col in range(width):
            prev = 0
            for row in range(count):
                p = row * width + col; x = data[p]; out[p] = (x ^ prev) if xor else ((x - prev) & 255); prev = x
        kind = STEP_COLUMN_XOR if xor else STEP_COLUMN_DELTA
        step = Step(kind, bytes((width,)) + _varint(count), bytes(out)); size = len(_encode([step]))
        if best is None or size < best[0]: best = size, step, bytes(out)
...
def _decode_step(s):
    if s.kind == STEP_RECORD_TRANSPOSE:
        ...
        if q != len(s.meta) or width < 2 or width > 64 or len(s.payload) != width * count: raise ValueError("invalid record transpose")
    ...
    if s.kind in (STEP_COLUMN_DELTA, STEP_COLUMN_XOR):
        ...
        width = s.meta[0]; count, q = _read_varint(s.meta, 1)
        if q != len(s.meta) or width < 2 or width > 64 or len(s.payload) != width * count: raise ValueError("invalid column transform")
        out = bytearray(len(s.payload))
        for col in range(width):
            prev = 0
            for row in range(count):
                p = row * width + col; x = s.payload[p]; out[p] = ((x + prev) & 255) if s.kind == STEP_COLUMN_DELTA else (x ^ prev); prev = x
        return bytes(out)
    if s.kind == STEP_V4: return v4.inverse(s.payload)
    return v4._decode_v4_step(s)

def inverse(blob):
    if len(blob) < 5 or blob[:3] != MAGIC or blob[3] != VERSION: raise ValueError("invalid structural v5 stream")
    count = blob[4]; p = 5
    if count == 0:
    ...
    for _ in range(count):
        ...
        kind = blob[p]; p += 1; mlen, p = _read_varint(blob, p); end = p + mlen
        if end > len(blob): raise ValueError("truncated metadata")
        meta = blob[p:end]; p = end; plen, p = _read_varint(blob, p); end = p + plen
...
def _v4_candidates(data, max_depth):
    return [Candidate(("v4:" + "+".join(c.kinds),), _encode([Step(STEP_V4, b"", c.blob)]), len(_encode([Step(STEP_V4, b"", c.blob)]))) for c in v4.analyze(data, max_depth=min(max_depth, 2))]
...
def analyze(data, max_depth=3):
    ...
    def add(steps, names):
        blob = _encode(steps)
        if blob not in seen: seen.add(blob); candidates.append(Candidate(tuple(names), blob, len(blob)))
    for name, fn in (("record-transpose", _record_transpose), ("column-delta", lambda x: _column_transform(x, False)), ("column-xor", lambda x: _column_transform(x, True))):
        ...
        step, payload = x; names = (name,); add([step], names); queue.append((payload, [step], names, 1))
    while queue:
    ...
    candidates.sort(key=lambda c: c.size); return candidates
...
def adaptive_transform(data, scorer, max_depth=3):
    raw = _raw(data); best_blob = raw; best_kinds = (); best = scorer(data)
