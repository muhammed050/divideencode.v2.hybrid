...
def word_split(data, w, s):
    ...
    The old implementation estimated the quotient width from a sparse sample.
    A later unsampled word could have a larger quotient and make ``to_bytes``
    overflow. We now collect the quotients in one pass, derive the exact width,
    and serialize them in a second lightweight pass. This is both exact and
    avoids the incorrect width assumption.
    """
    n = len(data)
    nw = n // w
    tail = data[nw * w:]
    ...
    for i, q in enumerate(qs):
        qb[i * wq:(i + 1) * wq] = q.to_bytes(wq, "little")
...
def classify(data):
    ...
    if n == 0:
        return {
            "n": 0, "printable": 0.0, "run_frac": 0.0, "big_run": False,
            "hi_zero2": 0.0, "hi_zero4": 0.0, "dist2": 256,
            "dist4": 256, "bd8_small": 0.0,
        }
    window = data[:65536]
    nonp = 0
    for b in window:
    ...
    return {
        "n": n, "printable": printable, "run_frac": run_frac,
        "big_run": big_run, "hi_zero2": hz2, "hi_zero4": hz4,
        "dist2": dist2, "dist4": dist4, "bd8_small": bd8,
    }
...
def plan_candidates(data):
    ...
    structured = (
        f["bd8_small"] > 0.30 or f["hi_zero4"] > 0.30 or f["hi_zero2"] > 0.45 or
        f["dist2"] <= 24 or f["dist4"] <= 12 or f["run_frac"] >= 0.02 or
        f["printable"] > 0.90)
...
__all__ = [
    ...
    "plane_pack", "plane_unpack", "word_split", "word_join", "classify",
    "plan_candidates", "METHOD_STORED", "METHOD_LZE", "METHOD_DELTA",
    "METHOD_RLE", "METHOD_STRUCT", "METHOD_WORD", "STRUCT_TEXT",
    "WORD_DEFAULT", "DEFAULT_WINDOW",
]
