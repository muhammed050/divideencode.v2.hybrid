...
# --- P2 search policy ------------------------------------------------------
# Single-level predictors cannot see multi-step wins (e.g. DELTA chains
# that only pay off two levels down). TRANSFORM_SLOP allows speculative
# expansion of options whose prediction is still within SLOP x best --
# each such expansion is itself beam-pruned, memoized and budget-capped,
# so the worst case stays bounded while deep wins are recovered.
ENABLE_SPECULATIVE_BEAM = True
TRANSFORM_SLOP = 2.5     # speculate while pred <= best * SLOP
RAW_SKIP_FRAC = 0.98     # never expand transforms predicted >= 0.98 * raw
DIVIDE_TIE_FRAC = 1.6    # sibling-divide cluster window (see third pass)
ENABLE_WORK_BUDGET = True
WORK_BUDGET_FACTOR = 90  # x input size, in bytes fed to expensive encoders
WORK_BUDGET_MIN = 1 << 20
# --------------------------------------------------------------------------
...
def encode_node(data, depth=DEFAULT_DEPTH, ctx=None):
    ...
    if hit is not None:
        return hit
    # Global search-work accounting, charged on entry so the total mirrors
    # the benchmark's "bytes through encode_node" metric (exhaustive runs
    # measure ~54x input; the default budget caps pathology above that).
    ctx.work_used += len(data)
    best = None
    exhaustive = SEARCH_MODE == "exhaustive"

    feats = ctx.features(data)
    n_ = len(data)
    # Expansion-class predictions (delta / divide / dict) are useless at
    # depth 0 -- there is nothing to expand -- and their sampled models
    # (dict mining, transform probes) dominated small-file encode times.
    want_expansions = depth > 0
    static_pairs = estimate_candidates(data) if (data and want_expansions) \
        else []
    preds = scoring.predict_all(data, feats, ctx, static_pairs,
                                expansions=want_expansions)
    ...
    if data:
        ...
        if depth > 0:
            ...
            def expand_dict():
                entries, escape = scoring.mine_dictionary(data, ctx)
                if not entries:
                    return None
            ...
            if exhaustive:
                ...
                # Per-class reservations: delta / divide / dict predictions
                # carry class-specific bias, so a pure top-K cut can crowd
                # out a winning class (e.g. dict on big.json). Each class
                # is guaranteed its best option within the slop envelope.
                reserved_classes = set()
                expanded = 0
                expanded_keys = set()
                first_divide_pred = None
                divide_won = False
                for key, pred, fn, args in options:
                    if expanded >= BEAM_K:
                        break
                    cls = key if isinstance(key, str) else "divide"
                    if pred is None:
                        pruned(key, "no-prediction")
                        continue
                    if pred >= RAW_SKIP_FRAC * (n + 1):
                        pruned(key, "over-raw")
                        continue
                    limit = len(best) * PRUNE_MARGIN
                    if pred > limit:
                        if not (ENABLE_SPECULATIVE_BEAM and ctx.budget_ok()
                                and pred <= len(best) * TRANSFORM_SLOP):
                            # do NOT break here: predictions carry per-class
                            # bias, a later pair may still be inside the slop
                            # envelope. Only stop once slop itself is exceeded.
                            if ENABLE_SPECULATIVE_BEAM:
                                pruned(key, "over-margin")
                                if pred > len(best) * TRANSFORM_SLOP:
                                    break
                                continue
                            pruned(key, "over-margin")
                            break  # sorted ascending: rest can only be worse
                    if not ctx.budget_ok():
                        pruned(key, "budget")
                        break
                    expanded += 1
                    reserved_classes.add(cls)
                    expanded_keys.add(key)
                    if cls == "divide" and first_divide_pred is None:
                        first_divide_pred = pred
                    blob = fn(*args)
                    if blob is not None:
                        before = len(best) if best is not None else None
                        consider(blob)
                        if cls == "divide" and before is not None and \
                                best is not None and len(best) < before:
                            divide_won = True
                        record(key, len(blob))
                    else:
                        pruned(key, "empty")

                # second pass: guarantee each unexpanded class its best
                # remaining candidate while the slop envelope holds.
                if ENABLE_SPECULATIVE_BEAM:
                    for key, pred, fn, args in options:
                        cls = key if isinstance(key, str) else "divide"
                        if cls in reserved_classes:
                            continue
                        if expanded >= BEAM_K + 2:
                            break
                        if pred is None or \
                                pred >= RAW_SKIP_FRAC * (n + 1) or \
                                pred > len(best) * TRANSFORM_SLOP:
                            continue
                        if not ctx.budget_ok():
                            pruned(key, "budget")
                            break
                    ...
                    # third pass -- divide tie-cluster: ONLY when a divide
                    # actually won this node. Sibling pairs of a winning
                    # divide get a real shot when their predictions cluster
                    # together; sampled child estimates go stale once one
                    # divide wins big. Restricting the pass to won nodes
                    # keeps the cascade off speculative dead ends.
                    if first_divide_pred is not None and divide_won:
                        for key, pred, fn, args in options:
                            if not isinstance(key, tuple) or \
                                    key in expanded_keys:
                                continue
                            if expanded >= BEAM_K + 3:
                                break
                            if pred is None or \
                                    pred > first_divide_pred * DIVIDE_TIE_FRAC:
                                continue
                            if not ctx.budget_ok():
                                pruned(key, "budget")
                                break
