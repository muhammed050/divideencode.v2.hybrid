...
def main():
    ...
    for name, data in workloads.items():
        ...
        if final > raw_de2:
            regressions += 1
        print(f"[{name}] selected={'+'.join(decision.kinds) or 'raw':<34} structural={decision.structural_size:8d} B final={final:8d} B DE2={raw_de2:8d} B delta={final-raw_de2:+d} B search={t2-t1:7.2f}s")
    print("-" * 118)
