...
def safe_score(blob):
    """Return DE2 size, or infinity when DE2 cannot encode a candidate.

    A structural candidate must be usable by the downstream codec.  DE2's
    word-split path can reject some transformed byte layouts with OverflowError;
    those candidates are invalid for this pipeline and must never crash the
    adaptive benchmark or beat a valid candidate.
    """
    try:
...
def main():
    ...
    for name, data in workloads.items():
        raw_de2 = safe_score(data)
        started = time.perf_counter()
        decision = adaptive_transform(data, scorer=safe_score, max_depth=3)
        elapsed = time.perf_counter() - started
        final = decision.downstream_size
        assert final is not None and final < sys.maxsize
