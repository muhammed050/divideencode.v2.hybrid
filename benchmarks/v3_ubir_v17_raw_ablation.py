...
def packed(name,b): return len(compress(b,block_size=1<<20,level='BALANCED'))

for name in ('json_small.json','json_large.json'):
    data=(CORPUS/name).read_bytes(); ir,classes,lanes=parse(data)
    print('\n'+'='*80+'\n'+name)
    print(f'V1.6 whole IR={len(ir):,} DE2={packed("ir",ir):,} B')
    names=['punct','dict','string','int','raw','const']
    for n,b in zip(names,lanes): print(f'{n:7s}: {len(b):9,} -> {packed(n,b):8,} B')

    # Raw-focused representations, semantics-independent micro-ablation.
    raw=lanes[4]
    try:
        vals=[]; q=0
        while q<len(raw): x,q=base._g(raw,q); vals.append(x)
        joined=b''.join(vals)
        # 1) length-prefixed raw (current)
        # 2) separator-free concatenation
        # 3) length bucket: encode lengths separately + concatenated bytes
        lengths=b''.join(base._u(len(x)) for x in vals)
        payload=lengths+joined
        print(f'raw variants: current={packed("raw",raw):,} concat={packed("concat",joined):,} lengths+concat={packed("lc",payload):,} B')
        # 4) first-byte/ASCII-class split while preserving bytes
        heads=bytes((x[0] if x else 0) for x in vals)
        tails=b''.join(x[1:] for x in vals)
        split=heads+tails
        print(f'raw split(heads+tails)={packed("split",split):,} B')
